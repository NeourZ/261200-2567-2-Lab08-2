 ...
 */
 ...
 public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
 {
     ...
     public static void main(String[] a) {
        Thread t1 = new Thread(new RandomLines());
        Thread t2 = new Thread(new RandomCircles());
        Thread t3 = new Thread(new RandomPolygons());
        t1.start();
        t2.start();
        t3.start();
    }
    ...
    // Random Lines
    static class RandomLines implements Runnable {
        public void run() {
            Turtle lineTurtle = new Turtle();
            lineTurtle.speed(20); // Fastest speed
            lineTurtle.width(2);
            for (int i = 0; i < 30; i++) {
                lineTurtle.penColor(getRandomColor()); // Random color
                lineTurtle.setPosition(getRandomX(), getRandomY()); // Random position
                lineTurtle.setDirection(Math.random() * 360); // Random direction
                lineTurtle.forward(Math.random() * 200); // Random length
            }
        }
    }
    
    // Random Circles
    static class RandomCircles implements Runnable {
        public void run() {
            Turtle circleTurtle = new Turtle();
            circleTurtle.speed(5);
            circleTurtle.width(1);
            for (int i = 0; i < 25; i++) {
                circleTurtle.penColor(getRandomColor());
                circleTurtle.setPosition(getRandomX(), getRandomY()); // Random position
                double radius = Math.random() * 50 + 10; // Random radius
                drawCircle(circleTurtle, radius);
            }
        }
    
        private void drawCircle(Turtle t, double radius) {
            double circumference = 2 * Math.PI * radius;
            int steps = 36; // Number of steps to draw a circle
    ...
    // Random Polygons
    static class RandomPolygons implements Runnable {
        public void run() {
            Turtle polygonTurtle = new Turtle();
            polygonTurtle.speed(20);
            polygonTurtle.width(2);
            for (int i = 0; i < 15; i++) {
                polygonTurtle.penColor(getRandomColor()); // Random color
                polygonTurtle.setPosition(getRandomX(), getRandomY()); // Random position
                int sides = (int) (Math.random() * 6) + 3; // Random number of sides (3 to 8)
                drawPolygon(polygonTurtle, sides, Math.random() * 50 + 20); // Random size
            }
        }
    
        private void drawPolygon(Turtle t, int sides, double sideLength) {
    ...
    private static String getRandomColor() {
        String[] colors = {"red", "orange", "yellow", "green", "blue", "purple", "pink", "cyan", "magenta"};
        return colors[(int) (Math.random() * colors.length)];
    }
    
    private static double getRandomX() {
        return Math.random() * 500 - 250; // Random x between -250 and 250
    }
    
    private static double getRandomY() {
        return Math.random() * 500 - 250; // Random y between -250 and 250
    }
